...
public:
	this(LanguagePass lp)
	{
		...
		Location loc;
		auto ft = buildFunctionTypeSmart(loc, buildVoid(loc));
		auto arrayType = buildArrayType(loc, ft);
		globalConstructors = buildArrayLiteralSmart(loc, arrayType);
		globalDestructors = buildArrayLiteralSmart(loc, arrayType);
		localConstructors = buildArrayLiteralSmart(loc, arrayType);
		localDestructors = buildArrayLiteralSmart(loc, arrayType);
	}
	...
	{
		auto var = buildVariableSmart(thisModule.loc, arr.type,
	...
	void handleTor(ir.ArrayLiteral arr, string name, ir.Function func)
	{
		...
		arr.exps ~= buildExpReference(func.loc, func);
	}
}
